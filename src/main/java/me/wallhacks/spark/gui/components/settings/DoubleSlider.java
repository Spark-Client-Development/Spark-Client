...
public class DoubleSlider extends SettingComponent {
    ...
    public DoubleSlider(DoubleSetting setting) {
        ...
        progress = setting.getValue() / (setting.getMax() - setting.getMin()) * 180;
        target = progress;
    }
    ...
    public int drawComponent(int posX, int posY, double deltaTime, int click, int mouseX, int mouseY) {
        ...
        if (sliding) {
            if (!Mouse.isButtonDown(0)) {
                ...
                value += setting.getSliderStep() * 0.5;
                value -= (value) % setting.getSliderStep();
