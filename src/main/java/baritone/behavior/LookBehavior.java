...
public final class LookBehavior extends Behavior implements ILookBehavior {
    ...
    public void updateTarget(Rotation target, boolean force) {
        this.target = target;
        this.force = force || !Baritone.settings().freeLook.getValue();
    }
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        boolean silent = Baritone.settings().antiCheatCompatibility.getValue() && !this.force;
        ...
        switch (event.getState()) {
            case PRE: {
                if (this.force) {
                    ...
                    ctx.player().rotationYaw += (Math.random() - 0.5) * Baritone.settings().randomLooking.getValue();
                    ctx.player().rotationPitch += (Math.random() - 0.5) * Baritone.settings().randomLooking.getValue();
                    if (desiredPitch == oldPitch && !Baritone.settings().freeLook.getValue()) {
                        nudgeToLevel();
                    }
                    this.target = null;
                }
    ...
    public void onPlayerRotationMove(RotationMoveEvent event) {
        if (this.target != null) {
            ...
            if (!Baritone.settings().antiCheatCompatibility.getValue() && event.getType() == RotationMoveEvent.Type.MOTION_UPDATE && !this.force) {
                this.target = null;
            }
        }
    }
