...
public final class InventoryBehavior extends Behavior {
    ...
    private int firstValidThrowaway() { // TODO offhand idk
        ...
        for (int i = 0; i < invy.size(); i++) {
            if (Baritone.settings().acceptableThrowawayItems.getValue().contains(invy.get(i).getItem())) {
                return i;
            }
        }
        return -1;
    ...
    private int bestToolAgainst(Block against, Class<? extends ItemTool> cla$$) {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (Baritone.settings().itemSaver.getValue() && stack.getItemDamage() >= stack.getMaxDamage() && stack.getMaxDamage() > 1) {
                continue;
            }
    ...
    public boolean hasGenericThrowaway() {
        for (Item item : Baritone.settings().acceptableThrowawayItems.getValue()) {
