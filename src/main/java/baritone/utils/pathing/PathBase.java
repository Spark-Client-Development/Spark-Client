...
public abstract class PathBase implements IPath {
    ...
    public PathBase cutoffAtLoadedChunks(Object bsi0) { // <-- cursed cursed cursed
        if (!Baritone.settings().cutoffAtLoadBoundary.getValue()) {
            return this;
        }
    ...
    public PathBase staticCutoff(Goal destination) {
        int min = BaritoneAPI.getSettings().pathCutoffMinimumLength.getValue();
        if (length() < min) {
            return this;
        }
        ...
        double factor = BaritoneAPI.getSettings().pathCutoffFactor.getValue();
