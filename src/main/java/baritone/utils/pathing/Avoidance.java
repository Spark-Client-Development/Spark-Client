...
public class Avoidance {
    ...
    public static List<Avoidance> create(IPlayerContext ctx) {
        if (!Baritone.settings().avoidance.getValue()) {
            return Collections.emptyList();
        }
        List<Avoidance> res = new ArrayList<>();
        double mobSpawnerCoeff = Baritone.settings().mobSpawnerAvoidanceCoefficient.getValue();
        double mobCoeff = Baritone.settings().mobAvoidanceCoefficient.getValue();
        if (mobSpawnerCoeff != 1.0D) {
            ctx.worldData().getCachedWorld().getLocationsOf("mob_spawner", 1, ctx.playerFeet().x, ctx.playerFeet().z, 2)
                    .forEach(mobspawner -> res.add(new Avoidance(mobspawner, mobSpawnerCoeff, Baritone.settings().mobSpawnerAvoidanceRadius.getValue())));
        }
        if (mobCoeff != 1.0D) {
            ctx.world().loadedEntityList.stream()
                    ...
                    .forEach(entity -> res.add(new Avoidance(new BlockPos(entity), mobCoeff, Baritone.settings().mobAvoidanceRadius.getValue())));
        }
        return res;
    }
