...
public interface MovementHelper extends ActionCosts, Helper {
    ...
    static boolean avoidAdjacentBreaking(BlockStateInterface bsi, int x, int y, int z, boolean directlyAbove) {
        ...
        if (!directlyAbove // it is fine to mine a block that has a falling block directly above, this (the cost of breaking the stacked fallings) is included in cost calculations
                ...
                && Baritone.settings().avoidUpdatingFallingBlocks.getValue() // and if the setting is enabled
    ...
    static boolean canWalkThrough(BlockStateInterface bsi, int x, int y, int z, IBlockState state) {
        ...
        if (Baritone.settings().blocksToAvoid.getValue().contains(block)) {
            return false;
        }
        ...
        if (block instanceof BlockLiquid) {
            if (Baritone.settings().assumeWalkOnWater.getValue()) {
                return false;
            }
    ...
    static boolean canWalkOn(BlockStateInterface bsi, int x, int y, int z, IBlockState state) {
        ...
        if (block == Blocks.LADDER || (block == Blocks.VINE && Baritone.settings().allowVines.getValue())) { // TODO reconsider this
            return true;
        }
        ...
        if (isWater(block)) {
            ...
            if (isFlowing(x, y, z, state, bsi) || block == Blocks.FLOWING_WATER) {
                ...
                return isWater(up) && !Baritone.settings().assumeWalkOnWater.getValue();
            }
            // if assumeWalkOnWater is on, we can only walk on water if there isn't water above it
            // if assumeWalkOnWater is off, we can only walk on water if there is water above it
            return isWater(up) ^ Baritone.settings().assumeWalkOnWater.getValue();
        }
        if (Baritone.settings().assumeWalkOnLava.getValue() && isLava(block) && !isFlowing(x, y, z, state, bsi)) {
            return true;
        }
        ...
        if (block instanceof BlockSlab) {
            if (!Baritone.settings().allowWalkOnBottomSlab.getValue()) {
    ...
    static void switchToBestToolFor(IPlayerContext ctx, IBlockState b) {
        switchToBestToolFor(ctx, b, new ToolSet(ctx.player()), BaritoneAPI.getSettings().preferSilkTouch.getValue());
    }
    ...
    static void switchToBestToolFor(IPlayerContext ctx, IBlockState b, ToolSet ts, boolean preferSilkTouch) {
        if (!Baritone.settings().disableAutoTool.getValue() && !Baritone.settings().assumeExternalAutoTool.getValue()) {
