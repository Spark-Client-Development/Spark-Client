...
public final class BuilderProcess extends BaritoneProcessHelper implements IBuilderProcess {
    ...
    public void build(String name, ISchematic schematic, Vec3i origin) {
        ...
        if (!Baritone.settings().buildSubstitutes.getValue().isEmpty()) {
            this.schematic = new SubstituteSchematic(this.schematic, Baritone.settings().buildSubstitutes.getValue());
        }
        int x = origin.getX();
        int y = origin.getY();
        int z = origin.getZ();
        if (Baritone.settings().schematicOrientationX.getValue()) {
            x += schematic.widthX();
        }
        if (Baritone.settings().schematicOrientationY.getValue()) {
            y += schematic.heightY();
        }
        if (Baritone.settings().schematicOrientationZ.getValue()) {
            z += schematic.lengthZ();
        }
        this.origin = new Vec3i(x, y, z);
        this.paused = false;
        this.layer = Baritone.settings().startAtLayer.getValue();
        this.numRepeats = 0;
        ...
        if (SchematicaHelper.isSchematicaPresent()) {
            ...
            if (schematic.isPresent()) {
                ...
                this.build(
                        ...
                        Baritone.settings().mapArtMode.getValue() ? new MapArtSchematic(s) : s,
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel, int recursions) {
        ...
        if (Baritone.settings().buildInLayers.getValue()) {
            ...
            if (Baritone.settings().layerOrder.getValue()) { // top to bottom
            ...
            for (int z = 0; z < schematic.lengthZ(); z++) {
                for (int x = 0; x < schematic.widthX(); x++) {
                    ...
                    if (!observedCompleted.contains(BetterBlockPos.longHash(blockX, blockY, blockZ))
                          && !Baritone.settings().buildSkipBlocks.getValue().contains(schematic.desiredState(x, y, z, current, this.approxPlaceable).getBlock())) {
    ...
    private Goal breakGoal(BlockPos pos, BuilderCalculationContext bcc) {
        if (Baritone.settings().goalBreakFromAbove.getValue() && bcc.bsi.get0(pos.up()).getBlock() instanceof BlockAir && bcc.bsi.get0(pos.up(2)).getBlock() instanceof BlockAir) { // TODO maybe possible without the up(2) check?
    ...
    public void onLostControl() {
        ...
        schematic = null;
        realSchematic = null;
        layer = Baritone.settings().startAtLayer.getValue();
        numRepeats = 0;
        paused = false;
    ...
    private boolean valid(IBlockState current, IBlockState desired, boolean itemVerify) {
        ...
        if (current.getBlock() instanceof BlockLiquid && Baritone.settings().okIfWater.getValue()) {
            return true;
        }
        if (current.getBlock() instanceof BlockAir && Baritone.settings().okIfAir.getValue().contains(desired.getBlock())) {
            return true;
        }
        if (desired.getBlock() instanceof BlockAir && Baritone.settings().buildIgnoreBlocks.getValue().contains(current.getBlock())) {
            return true;
        }
        if (!(current.getBlock() instanceof BlockAir) && Baritone.settings().buildIgnoreExisting.getValue() && !itemVerify) {
            return true;
        }
        if (Baritone.settings().buildSkipBlocks.getValue().contains(desired.getBlock()) && !itemVerify) {
            return true;
        }
        if (Baritone.settings().buildValidSubstitutes.getValue().getOrDefault(desired.getBlock(), Collections.emptyList()).contains(current.getBlock()) && !itemVerify) {
            return true;
        }
    ...
    public class BuilderCalculationContext extends CalculationContext {
        ...
        public double costOfPlacingAt(int x, int y, int z, IBlockState current) {
            ...
            if (sch != null && !Baritone.settings().buildSkipBlocks.getValue().contains(sch.getBlock())) {
        ...
        public double breakCostMultiplierAt(int x, int y, int z, IBlockState current) {
            ...
            if (sch != null && !Baritone.settings().buildSkipBlocks.getValue().contains(sch.getBlock())) {
                ...
                if (valid(bsi.get0(x, y, z), sch, false)) {
                    return Baritone.settings().breakCorrectBlockPenaltyMultiplier.getValue();
                } else {
