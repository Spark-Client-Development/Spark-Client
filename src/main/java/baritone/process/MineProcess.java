...
public final class MineProcess extends BaritoneProcessHelper implements IMineProcess {
    ...
    private boolean internalMiningGoal(BlockPos pos, CalculationContext context, List<BlockPos> locs) {
        ...
        if (Baritone.settings().internalMiningAirException.getValue() && state.getBlock() instanceof BlockAir) {
            return true;
        }
    ...
    private Goal coalesce(BlockPos loc, List<BlockPos> locs, CalculationContext context) {
        ...
        if (!Baritone.settings().forceInternalMining.getValue()) {
    ...
    public List<BlockPos> droppedItemsScan() {
        if (!Baritone.settings().mineScanDroppedItems.getValue()) {
    ...
    public static List<BlockPos> searchWorld(CalculationContext ctx, BlockOptionalMetaLookup filter, int max, List<BlockPos> alreadyKnown, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        for (BlockOptionalMeta bom : filter.blocks()) {
            ...
            if (CachedChunk.BLOCKS_TO_KEEP_TRACK_OF.contains(block)) {
                ...
                locs.addAll(ctx.worldData.getCachedWorld().getLocationsOf(
                        ...
                        Baritone.settings().maxCachedWorldScanCount.getValue(),
                        pf.x,
                        pf.z,
                        2
                ));
        ...
        if (!untracked.isEmpty() || (Baritone.settings().extendCacheOnThreshold.getValue() && locs.size() < max)) {
            ...
            for (int y = playerFeet.getY() - searchDist; y <= playerFeet.getY() + searchDist; y++) {
                for (int z = playerFeet.getZ() - searchDist; z <= playerFeet.getZ() + searchDist; z++) {
                    ...
                    if (filter.has(bsi.get0(x, y, z))) {
                        ...
                        if ((Baritone.settings().legitMineIncludeDiagonals.getValue() && knownOreLocations.stream().anyMatch(ore -> ore.distanceSq(pos) <= 2 /* sq means this is pytha dist <= sqrt(2) */)) || RotationUtils.reachable(ctx.player(), pos, fakedBlockReachDistance).isPresent()) {
    ...
    private static List<BlockPos> prune(CalculationContext ctx, List<BlockPos> locs2, BlockOptionalMetaLookup filter, int max, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        List<BlockPos> locs = locs2
                ...
                .filter(pos -> pos.getY() >= Baritone.settings().minYLevelWhileMining.getValue())
    ...
    public static boolean isNextToAir(CalculationContext ctx, BlockPos pos) {
        int radius = Baritone.settings().allowOnlyExposedOresDistance.getValue();
